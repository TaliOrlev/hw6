...
linked_list_node_t *create_new_node (char *node_data, int key);
void insert_node_list (linked_list_t *list, linked_list_node_t *new_node);
void change_duplicate (linked_list_t *list);
void free_linked_list (linked_list_node_t *node);
int cmp_lists (linked_list_t *list_note, linked_list_t *list_magazine);
...
void checkMagazine(int magazine_count, char** magazine, int note_count, char** note) {
    ...
    linked_list_t *list_magazine=(linked_list_t*)(malloc(sizeof(linked_list_t)));
    if (list_magazine==NULL) {
        free(list_magazine);
        return;
    }

    linked_list_t *list_note=(linked_list_t*)(malloc(sizeof(linked_list_t)));
    if (list_note==NULL) {
        free(list_magazine);
        free(list_note);
        return;
...
void change_duplicate (linked_list_t *list) {
    linked_list_node_t *rv = list->head;
    linked_list_node_t *rv2;
    linked_list_node_t *tmp;
    while (rv != NULL) {
        rv2=rv;
        while (rv2->next != NULL) {
            /* if strings are equal in a list, keep one node and change key */
...
int cmp_lists(linked_list_t *list_note, linked_list_t *list_magazine) {
    int word_was_found;
    linked_list_node_t *magazine_node;
    linked_list_node_t *note_node = list_note->head;
